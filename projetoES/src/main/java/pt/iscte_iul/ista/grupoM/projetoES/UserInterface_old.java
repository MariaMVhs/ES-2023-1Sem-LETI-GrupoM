...
public class UserInterface_old extends JFrame{

	private JPanel pathWindow;
	...
	public UserInterface_old() {
		pathWindow = new JPanel();
...
//		pathWindow.setVisible(false);
	...
	private void openFieldsWindow2() {
	...
	private void openRatingWindow() {
		JFrame ratingWindow = new JFrame("Avaliação do Horário");
		ratingWindow.setDefaultCloseOperation(WindowConstants.EXIT_ON_CLOSE);
		ratingWindow.setSize(200, 400);
		ratingWindow.setLocation(100, 50);
		ratingWindow.setLayout(new GridLayout(0, 1));
		setupRatingWindowButtons(ratingWindow);
		ratingWindow.setVisible(true);
	}
	
	private void setupRatingWindowButtons(JFrame frame) {
		HorarioRater rater=new HorarioRater(new Horario());
		JButton showRating = new JButton("Mostrar avaliação");
		JButton changeMetricas = new JButton("Alterar métricas");
		frame.add(showRating);
		frame.add(changeMetricas);
...
//	Buttons
	...
	//funcao para carregar o path do ficherio fornecido
	 private void showPathInputDialog() throws Exception {
		 ...
		 String path = JOptionPane.showInputDialog(UserInterface_old.this, "Enter file path:");
